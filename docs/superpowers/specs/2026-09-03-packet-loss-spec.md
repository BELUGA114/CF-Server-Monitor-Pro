...
四个目标**并行**执行 `ping -c 5 -w 6 -q`,各自输出写 `$ICMP_DIR/icmp-{ct,cu,cm,bd}`,`wait` 后统一解析,解析完删除。最坏耗时约 6 秒(全部超时),典型约 4 秒。

统计文件不能放 `/tmp`。agent 以 root 运行,`/tmp` 全局可写,非特权本地用户可以提前把 `/tmp/<可预测文件名>` 建成指向任意路径的符号链接,root 的 `>` 重定向跟随后会截断并覆写该目标(CWE-59)。因此启动时按 `/run/cf-probe`、`/var/lib/cf-probe` 顺序建目录(`umask 077` 加显式 `chmod 700`),两者的父目录都只有 root 可写;都建不出来则 `ICMP_OK=0`,退回纯 HTTP 探测并把丢包报 -1。安全性来自父目录只有 root 可写,`700` 属于额外加固。
...
- 存量 agent 需要重装。`/update` 只下发 `INTERVAL`、`CT`、`CU`、`CM`,没有脚本自更新通道,已装机器上的旧脚本不会上报 `loss_*`,面板会一直显示 `--`,延迟仍是 HTTP 口径。每台机器需重跑一键安装命令。
